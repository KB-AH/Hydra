...
uint8_t Start(void) {
    ...
    HAL_GPIO_WritePin(DS28E18_PORT, DS28E18_PIN,
                      GPIO_PIN_RESET);  // pull the pin low
    delay_us(480);
...
void set2SPI(void)  // Check seq = 0
{
    ...
    Write(0x02);
    Write(0x55);
    Write(0x38);  // least HEX char: 0 - 100kHz Speed, B - 2.3MHz speed 8 -
                  // 400kHz Speed
    bufferOW[6] = Read();
    bufferOW[7] = Read();
    Write(0xAA);
...
uint16_t run_OW(void) {
    ...
    //			HAL_UART_Transmit(&huart3, (uint8_t*)huart, sprintf(huart, "data  =
    //%d\n", retval), 20);
    // prev_data = datafin0;
    return datafin0;
}
